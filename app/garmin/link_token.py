"""Signed, time-limited tokens for the /garmin-login link sent by the bot.

A user gets this link only by asking the bot itself (/garmin_link), which already
knows their real Telegram id - so the link doesn't need the user to know or type
their own id, and a stranger can't construct a valid one for someone else's id
without the HMAC secret. Time-limited (not single-use) is enough for this app's
trust model: a small group behind a private tunnel/NAS reverse proxy, not the open
internet.
"""
